...
document.getElementById('kpi-metrics').textContent = DB_DATA.metrics_count.toLocaleString();
document.getElementById('gen-time').textContent = 'Updated: ' + DB_DATA.generated_at;
...
new Chart(document.getElementById('chartStates'), {{
  ...
  data: {{
    ...
    datasets: [{{
      label: 'Hospitals',
      data: DB_DATA.top_states.map(d => d.hospitals),
...
new Chart(document.getElementById('chartRating'), {{
  type: 'doughnut',
  data: {{
    labels: DB_DATA.rating_dist.map(d => d.rating + ' Star'),
    datasets: [{{
      data: DB_DATA.rating_dist.map(d => d.count),
...
new Chart(document.getElementById('chartTypes'), {{
  ...
  data: {{
    ...
    datasets: [{{
      label: 'Count',
      data: DB_DATA.hospital_types.map(d => d.count),
...
new Chart(document.getElementById('chartNational'), {{
  type: 'doughnut',
  data: {{
    labels: DB_DATA.national_comparison.map(d => d.result),
    datasets: [{{
      data: DB_DATA.national_comparison.map(d => d.count),
      backgroundColor: DB_DATA.national_comparison.map(d => natColors[d.result]||'#64748b'),
      borderWidth: 2, borderColor: '#1e293b',
    }}]
  }},
  options: {{
