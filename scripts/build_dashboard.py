...
snapshot_path = os.getenv("DASHBOARD_DATA_FILE")
if snapshot_path:
    print(f"Building from cached snapshot: {snapshot_path}")
...
.topbar-links{{display:flex;align-items:center;gap:10px;margin-left:auto}}
.topbar-links a{{color:#64748b;display:flex;align-items:center;transition:color .15s}}
.topbar-links a:hover{{color:#e2e8f0}}
.manual-refresh{{border:1px solid var(--border);border-radius:6px;padding:5px 8px;color:var(--muted)!important;font-size:11px;font-weight:600;gap:5px}}
.manual-refresh:hover{{color:#60a5fa!important;border-color:#3b82f6;background:rgba(59,130,246,.12)}}
.manual-refresh svg{{width:14px;height:14px}}
...
@media(max-width:680px){{
  ...
  .theme-toggle{{background:none;border:none;min-width:44px;min-height:44px;padding:6px;color:#64748b;display:flex;align-items:center;justify-content:center}}
  ...
  .btn-export-lg{{min-height:44px;display:inline-flex;align-items:center;padding:8px 14px;font-size:11px}}
  .btn-export{{min-height:36px;padding:6px 12px}}
}}
</style>
</head>
<body>
...
<main>
  <div class="topbar">
    ...
    <div class="topbar-links">
      <a class="manual-refresh" href="https://github.com/scale600/dba-azure-project/actions/workflows/deploy-dashboard.yml" target="_blank" rel="noopener noreferrer" title="Refresh the dashboard snapshot from GitHub Actions">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="23 4 23 10 17 10"/><polyline points="1 20 1 14 7 14"/><path d="M3.51 9a9 9 0 0 1 14.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0 0 20.49 15"/></svg>
        <span class="refresh-label">Manual refresh</span>
      </a>
